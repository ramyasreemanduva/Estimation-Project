...
def plot_trajectory(true_states, estimates, beacons, inner, outer):
    ...
    # 2. Lateral Position yt (Validation of Fig 3)
    # Min distance to true centerline [cite: 17]
    ...
    plt.axhline(y=2, color='r', linestyle='--', label='Bound (2m)') # Track width is 4m [cite: 14]
    plt.ylim(-0.5, 3); plt.grid(True); plt.ylabel("Position (m)"); plt.xlabel("Time (s)")

    # 3. Velocity (Constraint: ~10m/s, Max 25m/s) 
